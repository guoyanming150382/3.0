...
//封装ajax
	$.ajax=function(obj){
		...
		oAjax.open(obj.method,obj.url,true);//（获取方式，获取地址 true异步请求 false同步请求）
		oAjax.send();
		oAjax.onreadystatechange=function(){//添加监听事件
			if(oAjax.readyState==4){//等于4时候才成功
				if(oAjax.status==200){//等于2时候进行成功处理
					obj.success(oAjax.responseText);
				}else{//否则进行失败处理s

					obj.error(oAjax.status);
				}
			}
		}
	}
